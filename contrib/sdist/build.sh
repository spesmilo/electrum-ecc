...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-ecc-builder-img \
    "$CONTRIB_SDIST"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    echo "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum_ecc_build/fresh_clone/electrum_ecc"
...
docker run -it \
    --name electrum-ecc-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum_ecc \
    --rm \
    --workdir /opt/electrum_ecc/contrib/sdist \
    electrum-ecc-builder-img \
    ./make_sdist.sh
